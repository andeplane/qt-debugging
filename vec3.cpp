...
void vec3::randomUniform(double min, double max)
{
    ...
    components[0] = min + delta*(rand()/RAND_MAX);
    components[1] = min + delta*(rand()/RAND_MAX);
    components[2] = min + delta*(rand()/RAND_MAX);
}
...
vec3 vec3::normalize()
{
    ...
    if(myLength > 0) {
        components[0] /= myLength;
        components[1] /= myLength;
        components[2] /= myLength;
    }
    return *this;
}
